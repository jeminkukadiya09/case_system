...
from users.views import signin as sign_in
...
def person_updateform(request,person_id):
    if request.user.is_authenticated:
        if request.method == "POST":
            first_name = request.POST.get('first_name')
            last_name = request.POST.get('last_name')
            ...
            postalcode = request.POST.get('postalcode')
            country = request.POST.get('country')
            # if len(first_name) and len(last_name) and len(telephone1) and len(email1) and len(street) and len(town) and len()
            if len(postalcode) == 0:
                postalcode = None

            Person.objects.filter(person_id=person_id).update(first_name=first_name,last_name=last_name,telephone1=telephone1,email1=email1,street=street,town=town,postalcode=postalcode,country=country)
            return redirect(people)

        person=Person.objects.get(person_id=person_id)    
        return render(request,'myapp/person_updateform.html',{'person':person})
    else:
        return redirect(sign_in)    
...
def case(request):
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if state_id == "new" or state_id == "processing" or state_id == "inactive" or state_id == "lost":
                pass
            else:    
                if customer_loss_evidence == 'True' and customer_signed_contract == 'True' and lawyer_sent_letter_to_vendor == 'True' and fee_paid_to_lawyer == 'True' and  lawyer_assigned == 'True' and lawyer_id != "select_lawyer":
                    data = Case.objects.get(case_id=case_id)
                    state = State.objects.get(state_name="waiting")
                    data.state_id = state
                    data.save()

                if customer_loss_evidence == 'True' and customer_signed_contract == 'True' and lawyer_sent_letter_to_vendor == 'True' and fee_paid_to_lawyer == 'True' and  lawyer_assigned == 'True' and lawyer_id != "select_lawyer" and lawsuit_has_been_filed == 'True':
                ...
                if customer_loss_evidence == 'True' and customer_signed_contract == 'True' and lawyer_sent_letter_to_vendor == 'True' and fee_paid_to_lawyer == 'True' and  lawyer_assigned == 'True' and lawyer_id != "select_lawyer" and lawsuit_has_been_filed == 'True' and lawsuit_won == 'True':
...
def people(request):
    if request.user.is_authenticated:
        person_detail = Person.objects.all().order_by('-person_id')
        paginator = Paginator(person_detail,20)
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)
        return render(request,'myapp/person_detail.html', {'page_obj':page_obj})
    else:
        return redirect(sign_in)
...
def all_customer(request):
    if request.user.is_authenticated:
        data = Customer.objects.all().order_by('-customer_id')
        paginator = Paginator(data,20)
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)
        return render(request,'myapp/all_customer.html',{'page_obj':page_obj})
    else:
        return redirect(sign_in)
...
def all_lawyer(request):
    if request.user.is_authenticated:
        data = Lawyer.objects.all().order_by('-lawyer_id')
        paginator = Paginator(data,20)
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)
        return render(request,'myapp/all_lawyer.html',{'page_obj':page_obj})  
    else:
        return redirect(sign_in)    
...
def alluser(request):
    if request.user.is_authenticated:
        ...
        paginator = Paginator(user1,20)
        page_number = request.GET.get('page')
        page_obj = paginator.get_page(page_number)
        return render(request,'myapp/user_data.html',{'page_obj':page_obj})
    else:
        return redirect(sign_in)      



def user_updateform(request,id):
    if request.user.is_authenticated:
        if request.method == "POST":
            user_type = request.POST.get('user_type')
            username = request.POST.get('username')
            User.objects.filter(id=id).update(user_type=user_type,username=username)
            return redirect(alluser)
        user_data = User.objects.get(id=id)
        return render(request,'myapp/user_updateform.html',{'user_data':user_data})
    else:
        return redirect(sign_in)      
...
def case_update(request, case_id):
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            if state_id == "new" or state_id == "processing" or state_id == "inactive" or state_id == "lost":
                data = Case.objects.get(case_id=case_id)
                data.customer_loss_evidence = False 
                data.customer_signed_contract = False
                data.lawyer_sent_letter_to_vendor =False
                data.fee_paid_to_lawyer = False
                data.lawyer_assigned = False
                data.lawsuit_has_been_filed =False
                data.lawsuit_won=False
                data.save()
            else:    
                if customer_loss_evidence == 'True' and customer_signed_contract == 'True' and lawyer_sent_letter_to_vendor == 'True' and fee_paid_to_lawyer == 'True' and  lawyer_assigned == 'True' and lawyer_id != "select_lawyer":
                    data = Case.objects.get(case_id=case_id)
...
def excel(request):
    if request.user.is_authenticated:
        ...
        font_style = xlwt.XFStyle()
        font_style.font.bold = True
        ws.write(row_num, 0, 'Earning_data', font_style)
        row_num += 1 
        col = ['id','TotalEarningOfAllCase', 'TotalEarningsOfAllSportsBetsCases','TotalEarningsOfAllSlotMachineCases','TotalEarningsOfAllOnlineCasinoCases','TotalEarningsOfAllWonCases','TotalAmountWonOfAllWonCases','TotalEaringsOfAllLostCases','TotalClaimAmountOfAllFiledLawsuitsCases','TotalCustomerAmountLossOfAllProcessingCases','TotalClaimAmountOfAllProcessingCases']
        # naive = dt.replace(tzinfo=None)
        for col_num in range(len(col)):
            ws.write(row_num, col_num, col[col_num], font_style) # at 0 row 0 column 
        # Sheet body, remaining rows
        font_style = xlwt.XFStyle()
        rows = Earning_data.objects.all().values_list('id','TotalEarningOfAllCase', 'TotalEarningsOfAllSportsBetsCases','TotalEarningsOfAllSlotMachineCases','TotalEarningsOfAllOnlineCasinoCases','TotalEarningsOfAllWonCases','TotalAmountWonOfAllWonCases','TotalEaringsOfAllLostCases','TotalClaimAmountOfAllFiledLawsuitsCases','TotalCustomerAmountLossOfAllProcessingCases','TotalClaimAmountOfAllProcessingCases')
        for row in rows:
            row_num += 1
            for col_num in range(len(row)):
                ws.write(row_num, col_num, row[col_num], font_style) 
...
def Earning(request):
    if request.user.is_authenticated:
        ...
        sum11 = Decimal(sum11)
        for i in data:
            sum11 = sum11 + i.earnings_from_claim
            print(i.earnings_from_claim)
        
        c = CurrencyRates()
        Currency = c.get_rate('INR', 'EUR') 
        print(Currency)
        sum11 = Currency * int(sum11)
        sum11 = round(sum11,2)    
        print("Total Earning Of All Case",sum11)   
        ...
        Earning_data.objects.create(TotalEarningOfAllCase=sum11,TotalEarningsOfAllSportsBetsCases=sum1,TotalEarningsOfAllSlotMachineCases=sum2,TotalEarningsOfAllOnlineCasinoCases=sum3,TotalEarningsOfAllWonCases=sum4,TotalAmountWonOfAllWonCases=sum5,TotalEaringsOfAllLostCases=sum6,TotalClaimAmountOfAllFiledLawsuitsCases=sum7,TotalCustomerAmountLossOfAllProcessingCases=sum8,TotalClaimAmountOfAllProcessingCases=sum9)

        return render(request,'myapp/earning.html',{'all_earnings':sum11,'sports_bets':sum1,'Slot_Machine':sum2,'Online_Casino':sum3 ,'Won':sum4,'Won_case_amount_won':sum5,'Lost':sum6,'Claim_Amount':sum7,'customer_amount_lost':sum8,'case_amount_claim':sum9})
    else:
        return redirect(sign_in)
